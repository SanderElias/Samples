 ...
 *
 * this is an adopted version of the original code, as it didn't land in the official release yet
 * I adopted it to be able to create some demos before it lands in the official release
 */
...
import type { Resource, ResourceLoader, ResourceOptions, ResourceStatus, WritableResource } from './api';
...
abstract class BaseWritableResource<T> implements WritableResource<T> {
  ...
  constructor(equal: ValueEqualityFn<T> | undefined) {
    ...
    this.value.update = (fn: (value: T | undefined) => T | undefined) => this.set(fn(untracked(this.value)));
  }
