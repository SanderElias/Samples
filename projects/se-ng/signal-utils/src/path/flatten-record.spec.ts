...
describe('flattenRecord', () => {
  ...
  it('flattens arrays of objects', () => {
    expect(flattenRecord({ a: [{ b: 1 }, { c: 2 }] })).toEqual({
      'a.0.b': 1,
      'a.1.c': 2
    });
  });
  ...
  it('marks circular when onCircular=mark and default marker used', () => {
    const a: any = { x: 1 };
    a.self = a;
    expect(flattenRecord(a, { onCircular: 'mark' })).toEqual({
      x: 1,
      self: '[Circular]'
    });
  });
  ...
  it('marks circular with custom marker when onCircular=mark and marker set', () => {
    const a: any = { x: 1 };
    a.self = a;
    expect(flattenRecord(a, { onCircular: 'mark', marker: 'CIRCLED' })).toEqual(
      { x: 1, self: 'CIRCLED' }
    );
  });
  ...
  it('marks circular arrays when onCircular=mark (self ref)', () => {
    const arr: any[] = [1];
    arr.push(arr);
    expect(flattenRecord({ a: arr }, { onCircular: 'mark' })).toEqual({
      'a.0': 1,
      'a.1': '[Circular]'
    });
  });
  ...
  it('marks complex array->object->array cycle using custom marker', () => {
    ...
    const out = flattenRecord(
      { root: arr },
      { onCircular: 'mark', marker: 'CIRCLED' }
    );
  ...
  it('works when called with an array root (marking self-ref)', () => {
    const arr: any[] = [1];
    arr.push(arr);
    expect(flattenRecord(arr as any, { onCircular: 'mark' })).toEqual({
      '0': 1,
      '1': '[Circular]'
    });
  });
  ...
  it('throws when ancestors already contains the root (throw)', () => {
    ...
    anc.add(obj);
    const internals = (flattenRecord as any).__test;
    expect(() => internals._flattenRecord(obj, '', anc, 'throw', 'X')).toThrow(
      /Circular/
    );
  });
  ...
  it('internal array early-throw honored via internals (throw)', () => {
    ...
    anc.add(arr);
    const internals = (flattenRecord as any).__test;
    expect(() => internals._flattenArray(arr, '', anc, 'throw', 'X')).toThrow(
      /Circular/
    );
  });
  ...
  it('throws with path info when onCircular=throw (nested)', () => {
    const b: any = { nested: { c: 2 } };
    b.nested.self = b.nested;
    expect(() => flattenRecord(b, { onCircular: 'throw' })).toThrow(
      /nested.self/
    );
  });
  ...
  it('throws on circular arrays when onCircular=throw', () => {
    const arr: any[] = [1];
    arr.push(arr);
    expect(() => flattenRecord({ a: arr }, { onCircular: 'throw' })).toThrow(
      /a.1/
    );
  });
});
