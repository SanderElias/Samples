...
// node_modules/.pnpm/mermaid@11.16.1/node_modules/mermaid/dist/chunks/mermaid.core/chunk-6Q2QTUOP.mjs
var diagramTitle = "";
var accTitle = "";
var accDescription = "";
var rules = [];
...
var sanitizeAstNode = /* @__PURE__ */ __name((node) => {
  switch (node.type) {
    case "terminal":
      return __spreadProps(__spreadValues({}, node), { value: sanitizeText2(node.value) });
    case "nonterminal":
      return __spreadProps(__spreadValues({}, node), { name: sanitizeText2(node.name) });
    case "sequence":
      return __spreadProps(__spreadValues({}, node), { elements: node.elements.map(sanitizeAstNode) });
    case "choice":
      return __spreadProps(__spreadValues({}, node), { alternatives: node.alternatives.map(sanitizeAstNode) });
    case "optional":
      return __spreadProps(__spreadValues({}, node), { element: sanitizeAstNode(node.element) });
    case "repetition":
    ...
    case "special":
      return __spreadProps(__spreadValues({}, node), { text: sanitizeText2(node.text) });
  }
}, "sanitizeAstNode");
...
var addRule = /* @__PURE__ */ __name((rule) => {
  const sanitizedRule = __spreadProps(__spreadValues({}, rule), {
...
var extractThemeOverrides = /* @__PURE__ */ __name((options) => {
  ...
  const _a = options, {
    ...
    theme: _theme,
    look: _look
  } = _a, themeOverrides = __objRest(_a, [
    "railroad",
    "svgId",
    "theme",
    "look"
...
var buildThemeDefaults = /* @__PURE__ */ __name((themeVariables) => {
  ...
  return __spreadProps(__spreadValues({}, DEFAULT_RAILROAD_CONFIG), {
    fontFamily,
    fontSize,
...
var buildRailroadStyleOptions = /* @__PURE__ */ __name((options) => {
  ...
  const themeVariables = __spreadValues(__spreadValues(__spreadValues({}, getThemeVariables3()), currentConfig.themeVariables ?? {}), extractThemeOverrides(options));
  const themeDefaults = buildThemeDefaults(themeVariables);
  const railroadOptions = __spreadValues(__spreadValues({}, currentConfig.railroad ?? {}), extractRailroadOverrides(options));
  return {
