...
var cachingEnabled = signal(
  false,
  ...ngDevMode ? [{ debugName: "cachingEnabled" }] : (
    /* istanbul ignore next */
    []
  )
);
var httpCachedOptions = computed(
  () => {
    if (cachingEnabled()) {
      console.log("Caching enabled for HTTP requests");
      return __spreadValues({}, addCachingContext({
        credentials: "include",
        mode: "cors",
  ...
  ...ngDevMode ? [{ debugName: "httpCachedOptions" }] : (
    /* istanbul ignore next */
    []
  )
);
