...
export class LsHomeComponent implements OnInit {
  ...
  id$ = this.route.params.pipe(pluck('id'), map(id => +id));

  clicks$ = this.localState$.pipe(
    pluck('buttons'),
  ...
  @ViewChildren('b', { read: ElementRef }) buttons: QueryList<HTMLButtonElement>;
  ...
  return (target: any, propertyName: string) => {
    Object.defineProperty(target, propertyName, {
      set: function (this: any, x) {
        ...
        if (x) { rs.next({ ...rs.value, [propertyName]: x }) }
      },
      get: function (this: any) { throw new Error(`Don't read the "${propertyName}" directly, instead take it from the "${input}" behaviourSubject`) },
      configurable: false,
      enumerable: true,
    });
