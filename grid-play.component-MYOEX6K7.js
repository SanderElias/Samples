  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _GridPlayComponent, selectors: [["se-grid-play"]], viewQuery: function GridPlayComponent_Query(rf, ctx) {
      ...
      if (rf & 2) {
        ...
        \u0275\u0275advance(3);
        \u0275\u0275styleProp("--%NS%x", 1)("--%NS%y", 1);
        \u0275\u0275advance(2);
        \u0275\u0275styleProp("--%NS%x", 20)("--%NS%y", 40);
        \u0275\u0275advance(2);
        \u0275\u0275styleProp("--%NS%x", ctx.mouseOnCell().x)("--%NS%y", ctx.mouseOnCell().y);
        \u0275\u0275advance(2);
        \u0275\u0275textInterpolate2("", ctx.mouseOnCell().x, ", ", ctx.mouseOnCell().y);
      }
    }, styles: ["\n[_nghost-%COMP%] {\n  display: block;\n  --%NS%cellSize: 15px;\n  --%NS%xCells: 25;\n  --%NS%yCells: 25;\n  --%NS%gap: 1px;\n  --%NS%accent1: #f082cb;\n  --%NS%accent2: #9c1d7c;\n}\n.mouse-indicator[_ngcontent-%COMP%] {\n  display: none;\n  width: calc(var(--%NS%cellSize) - (2 * var(--%NS%gap)));\n  height: calc(var(--%NS%cellSize) - (2 * var(--%NS%gap)));\n  pointer-events: none;\n  border: 2px solid black;\n  border-radius: 4px;\n  box-sizing: border-box;\n  grid-column-start: var(--%NS%x);\n  grid-row-start: var(--%NS%y);\n}\n.grid-container[_ngcontent-%COMP%] {\n  display: grid;\n  inline-size: calc(var(--%NS%xCells) * var(--%NS%cellSize) + (var(--%NS%xCells) - 1) * var(--%NS%gap));\n  block-size: calc(var(--%NS%yCells) * var(--%NS%cellSize) + (var(--%NS%yCells) - 1) * var(--%NS%gap));\n  grid-template-columns: repeat(var(--%NS%xCells), var(--%NS%cellSize));\n  grid-template-rows: repeat(var(--%NS%yCells), var(--%NS%cellSize));\n  gap: var(--%NS%gap);\n  align-items: center;\n  justify-items: center;\n  border: 1px solid white;\n  background-image:\n    repeating-linear-gradient(\n      to right,\n      white 0 var(--%NS%gap),\n      rgba(255, 255, 255, 0) var(--%NS%gap) calc(var(--%NS%cellSize) + var(--%NS%gap))),\n    repeating-linear-gradient(\n      to bottom,\n      white 0 var(--%NS%gap),\n      rgba(255, 255, 255, 0) var(--%NS%gap) calc(var(--%NS%cellSize) + var(--%NS%gap))),\n    linear-gradient(\n      135deg,\n      var(--%NS%accent1) 0%,\n      var(--%NS%accent2) 100%);\n  background-position:\n    calc(var(--%NS%cellSize)) 0,\n    0 calc(var(--%NS%cellSize)),\n    0 0;\n  background-repeat:\n    repeat,\n    repeat,\n    no-repeat;\n  background-size: auto;\n  background-clip: padding-box;\n}\n.grid-container[_ngcontent-%COMP%]:hover   .mouse-indicator[_ngcontent-%COMP%] {\n  display: block;\n}\n.grid-item[_ngcontent-%COMP%] {\n  --%NS%x: attr(data-x type(<number>));\n  --%NS%y: attr(data-y type(<number>));\n  font-weight: bold;\n  font-size: calc(var(--%NS%cellSize) * 0.95);\n  color: white;\n  text-shadow: 0 0 3px rgba(0, 0, 0, 0.5);\n  border-radius: 4px;\n  grid-column-start: var(--%NS%x);\n  grid-row-start: var(--%NS%y);\n}"] });
  }
};
(() => {
