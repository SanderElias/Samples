import {
  clear as clear4
} from "./chunk-5RNHJPYS.js";
import {
  Graph
} from "./chunk-GSBJT4VC.js";
import {
  clear as clear3,
  edgeLabels,
  insertEdge,
...
import {
  ...
  insertNode,
  positionNode
} from "./chunk-P36NFHOR.js";
import "./chunk-SD42X4BU.js";
import "./chunk-ONDIXXFL.js";
import {
  markerOffsets
} from "./chunk-NEY7OAJU.js";
import {
  getSubGraphTitleMargins
} from "./chunk-VTZT6Y6A.js";
import "./chunk-ABZ6J6OS.js";
import "./chunk-752GXJTC.js";
import {
  utils_default
...
// node_modules/.pnpm/mermaid@11.16.1/node_modules/mermaid/dist/chunks/mermaid.core/swimlanes-SLNWSIFB.mjs
async function createGraphWithElements(element, data4Layout) {
  ...
  if (globalThis.mermaidCaptureSizes) {
    const { captureNodeSizes } = await import("./sizeCapture-X5ZJPWSS-GW5UHU2L.js");
...
function applyMarkerOffsets(points, edge) {
  if (points.length < 2) {
    return points.map((p) => __spreadValues({}, p));
  }
  const out = points.map((p) => __spreadValues({}, p));
...
function applyLineJumpsToSvg(edgePathsGroup, edges, config) {
  ...
  for (const e of edges) {
    ...
    renderedEdges.push(__spreadProps(__spreadValues({}, e), { points }));
  }
...
function createEdgeLabelNodes(data) {
  ...
  for (const edge of data.edges) {
    ...
    const labelNode = __spreadValues({
      id: labelNodeId,
      label: edge.label,
...
function moveCollinearEndpointRun(points, endpointIndex, step, endpoint, adjusted, horizontalTerminal) {
  const next = points.map((point) => __spreadValues({}, point));
...
function prepareEdgeEndpointsForRenderer(edges, nodeByIdMap) {
  for (const edge of edges) {
    ...
    const duplicated = [
      newPts[0],
      __spreadValues({}, newPts[0]),
      ...newPts.slice(1, -1),
      newPts[newPts.length - 1],
      __spreadValues({}, newPts[newPts.length - 1])
    ];
...
function mirrorAxis(layout, axis) {
  ...
  for (const node of nodes) {
    ...
    if (titleRect) {
      node.groupTitleRect = axis === "x" ? __spreadProps(__spreadValues({}, titleRect), {
        left: mirror(titleRect.right),
        right: mirror(titleRect.left)
      }) : __spreadProps(__spreadValues({}, titleRect), {
      ...
      for (let j = i + 1; j < lanes.length && !fixed; j++) {
        ...
        for (const lane of candidates) {
          for (const shift of shifts) {
            ...
            const nextLane = terminalLaneFor(__spreadProps(__spreadValues({}, lane.edge), { points: candidate }), lane.atStart);
...
function liftObstacleHuggingSameSideRails(edges, nodeByIdMap) {
  ...
  const candidateByMovingRail = /* @__PURE__ */ __name((points, rail, coord) => {
    const candidate = points.map((point) => __spreadValues({}, point));
    if (rail.horizontal) {
...
function liftTopLaneTitleBandsAboveRails(edges, nodeByIdMap) {
  ...
  for (const lane of lanes) {
    ...
    lane.node.groupTitleRect = __spreadProps(__spreadValues({}, lane.rect), {
...
function straightenCollinearSiblingDetours(edges, nodes) {
  ...
  for (const edge of edges) {
    ...
    for (const delta of deltas) {
      const shiftedSrc = __spreadValues({}, targetSrc);
      const shiftedDst = __spreadValues({}, targetDst);
      if (collinearX) {
        shiftedSrc.x += delta;
        shiftedDst.x += delta;
...
function nudgeSharedInteriorSubpaths(edges, nodeByIdMap) {
  ...
  const segmentsFor2 = /* @__PURE__ */ __name((edge, points) => {
    return orthogonalSegmentsForPoints(points, EPS_LOCAL2).map((segment) => __spreadProps(__spreadValues({}, segment), {
      edge,
  ...
  const shiftedCandidate = /* @__PURE__ */ __name((segment, shift) => {
    ...
    const candidate = points.map((p) => __spreadValues({}, p));
    if (segment.horizontal) {
...
function optimizeRanksByCrossings(g, initialRank) {
  const rankOf = __spreadValues({}, initialRank);
  const { preds } = buildPredecessorSuccessorMaps(g);
...
function assignLayers_Gravity(gAcyclic, opts) {
  ...
  const rankOf = __spreadValues({}, base.rankOf);
  const topLaneOf = createTopLaneResolver(g);
...
function routeEdgesOrthogonal(data, direction) {
  ...
  for (const oe of originalEdges) {
    ...
    edges.push(__spreadProps(__spreadValues({}, oe), {
      __originalEdge: oe
    }));
  }
  const nodeById = /* @__PURE__ */ new Map();
  ...
  for (const i of routingOrder) {
    ...
    const pSrcAnchor = __spreadValues({}, pSrcPort);
    const pDstAnchor = __spreadValues({}, pDstPort);
    ...
    if (srcHandleWaypoints.length === 0 && dstHandleWaypoints.length === 0) {
      ...
      if ((anchorsSameX || anchorsSameY) && !hasPortOffset && (!faceContested || canPreserveSimpleContestedStraight)) {
        ...
        if (!directBlocked) {
          e.points = [__spreadValues({}, pSrcPort), __spreadValues({}, pSrcAnchor), __spreadValues({}, pDstAnchor), __spreadValues({}, pDstPort)];
